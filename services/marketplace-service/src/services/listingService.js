...
async function buyListing(listingId, buyerId) {
    ...
    const transferResult = await apiClient.transferCredits(listing.userId, buyerId, listing.amount);
    ...
    const result = await data_source_1.AppDataSource.transaction(async (manager) => {
        ...
        const order = await orderService.createOrder(buyerId, listing.userId, listing.amount, listing.pricePerCredit, listing, manager);
