...
exports.getBidsHandler = getBidsHandler;
exports.closeAuctionHandler = closeAuctionHandler;
...
async function getBidsHandler(req, res) {
    const listingId = req.params.id;
    const bids = await bidService.getBidsForListing(listingId);
    res.json(bids);
}
async function closeAuctionHandler(req, res) {
    try {
        const listingId = req.params.id;
