...
exports.getCO2Savings = exports.getVehicleTrips = exports.addTrip = void 0;
...
const addTrip = async (req, res) => {
    try {
        ...
        const { error, value } = trip_validator_1.addTripSchema.validate(req.body, {
            abortEarly: false,
        });
        if (error) {
            res.status(400).json({
        ...
        const vehicle = await vehicle_model_1.Vehicle.findOne({
            _id: vehicleId,
            user_id: userId,
        });
        if (!vehicle) {
...
const getVehicleTrips = async (req, res) => {
    try {
        ...
        const sortBy = req.query.sort || '-start_time'; // Default: newest first
        // Find vehicle
        const vehicle = await vehicle_model_1.Vehicle.findOne({
            _id: vehicleId,
            user_id: userId,
...
const getCO2Savings = async (req, res) => {
    try {
        ...
        const { error, value } = trip_validator_1.getTripStatsSchema.validate(req.query, {
            abortEarly: false,
        });
        if (error) {
            res.status(400).json({
        ...
        if (period === 'monthly' && year && month) {
            ...
            filteredTrips = vehicle.trips.filter((trip) => new Date(trip.start_time) >= startDate &&
        ...
        const totalDistance = filteredTrips.reduce((sum, trip) => sum + trip.distance_km, 0);
        const totalCO2Saved = filteredTrips.reduce((sum, trip) => sum + trip.co2_saved_kg, 0);
        ...
        if (period === 'yearly' && year) {
            for (let m = 1; m <= 12; m++) {
                ...
                const monthTrips = vehicle.trips.filter((trip) => new Date(trip.start_time) >= monthStart &&
                    new Date(trip.start_time) <= monthEnd);
                const monthDistance = monthTrips.reduce((sum, trip) => sum + trip.distance_km, 0);
                const monthCO2 = monthTrips.reduce((sum, trip) => sum + trip.co2_saved_kg, 0);
...
exports.getCO2Savings = getCO2Savings;
