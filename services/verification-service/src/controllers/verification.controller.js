...
class VerificationController {
    constructor() {
        this.verificationRepository = database_1.AppDataSource.getRepository(Verification_1.Verification);
        this.kycRepository = database_1.AppDataSource.getRepository(KYC_1.KYC);
        this.certificateRepository = database_1.AppDataSource.getRepository(Certificate_1.Certificate);
        ...
        this.getPendingVerifications = async (req, res) => {
            try {
                ...
                const [verifications, total] = await this.verificationRepository.findAndCount({
                    where: { status: Verification_1.VerificationStatus.PENDING },
        ...
        this.approveVerification = async (req, res) => {
            try {
                ...
                if (verification.status !== Verification_1.VerificationStatus.PENDING) {
                    res.status(400).json({
                        success: false,
                ...
                verification.status = Verification_1.VerificationStatus.APPROVED;
                ...
                certificate.id = (0, uuid_1.v4)();
                certificate.user_id = verification.user_id;
                certificate.verification_id = verification.id;
                certificate.type = Certificate_1.CertificateType.CARBON_CREDIT;
        ...
        this.rejectVerification = async (req, res) => {
            try {
                ...
                verification.status = Verification_1.VerificationStatus.REJECTED;
                verification.cva_id = cva_id;
                verification.notes = notes;
                verification.reviewed_at = new Date();
                await this.verificationRepository.save(verification);
        ...
        this.submitKYC = async (req, res) => {
            try {
                ...
                if (existingKYC && existingKYC.status === KYC_1.KYCStatus.APPROVED) {
                    res.status(400).json({
                        success: false,
                ...
                kyc.status = KYC_1.KYCStatus.PENDING;
        ...
        // Get KYC status
        this.getKYCStatus = async (req, res) => {
            try {
                const { userId } = req.params;
        ...
        this.verifyCredits = async (req, res) => {
            try {
                ...
                verification.status = Verification_1.VerificationStatus.PENDING;
                await this.verificationRepository.save(verification);
                res.status(201).json({
                    success: true,
                    data: {
        ...
        this.approveCredits = async (req, res) => {
            try {
                const { verification_id, cva_id, notes, credits_amount } = req.body;
                const verification = await this.verificationRepository.findOne({
                    where: { id: verification_id }
        ...
        this.rejectCredits = async (req, res) => {
            try {
                const { verification_id, cva_id, notes } = req.body;
                const verification = await this.verificationRepository.findOne({
                    where: { id: verification_id }
        ...
        this.getUserCertificates = async (req, res) => {
            try {
                const { userId } = req.params;
        ...
        this.getCertificatesForCurrentUser = async (req, res) => {
            try {
        ...
        this.downloadCertificatePdf = async (req, res) => {
            try {
                const { id } = req.params;
        ...
        this.createCertificateShareLink = async (req, res) => {
            try {
                const { id } = req.params;
                const cert = await this.certificateRepository.findOne({ where: { id } });
                if (!cert) {
        ...
        this.generateVerificationReport = async (req, res) => {
            try {
                ...
                const approvedVerifications = await query
                    .clone()
                    .andWhere('verification.status = :status', { status: Verification_1.VerificationStatus.APPROVED })
                    .getCount();
                const rejectedVerifications = await query
                    .clone()
                    .andWhere('verification.status = :status', { status: Verification_1.VerificationStatus.REJECTED })
                    .getCount();
                const pendingVerifications = await query
                    .clone()
                    .andWhere('verification.status = :status', { status: Verification_1.VerificationStatus.PENDING })
                    .getCount();
                ...
                let approvedQuery = this.verificationRepository
                    ...
                    .where('verification.status = :status', { status: Verification_1.VerificationStatus.APPROVED });
                if (start_date && end_date) {
                ...
                const approvedKYC = await this.kycRepository.count({
                    where: { status: KYC_1.KYCStatus.APPROVED }
                });
                // Get certificates issued
                const certificatesIssued = await this.certificateRepository.count({
                    where: {
                        type: Certificate_1.CertificateType.CARBON_CREDIT
                    }
                });
                res.json({
                    success: true,
