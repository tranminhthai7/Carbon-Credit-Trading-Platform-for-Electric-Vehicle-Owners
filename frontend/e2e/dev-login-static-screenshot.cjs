...
async function main() {
  ...
  // Wait for navigation to dashboard to finish, then screenshot
  ...
  if (finalUrl.includes('/owner/dashboard') || finalUrl.includes('/dashboard')) {
    ...
    await page.screenshot({ path: './e2e/emily-dashboard.png', fullPage: true });
    await browser.close();
    process.exit(0);
  } else {
    console.error('E2E FAIL: final url', finalUrl);
    await page.screenshot({ path: './e2e/fail-static-screenshot.png' });
    await browser.close();
    process.exit(1);
  }
}
