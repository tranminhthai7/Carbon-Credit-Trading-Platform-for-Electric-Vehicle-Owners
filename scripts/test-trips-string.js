 (async () => {
  ...
  const email = `trips-node-str+${stamp}@example.com`;
  const pw = 'Testpass123!';
  console.log('Registering', email);
  try { await (await fetch(`${gateway}/api/users/register`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ email, password: pw, role: 'ev_owner', full_name: 'Trips Node String', phone: '+84900000005' }) })).text(); } catch(e) { console.warn('register may have failed', e.message) }
  ...
  const tripPayload = {
    ...
    // send the locations as strings instead of objects to reproduce client behavior
    start_location: '10.7758,106.7004',
    end_location: '10.7767 106.7021'
  };
  try {
